...
export default class Component extends HTMLElement {
   ...
   * @param {Boolean} once 
   */
  static mound(point = document.body, once = true) {
    if (point instanceof HTMLElement && point.insertAdjacentHTML) {
      if (once && document.querySelector(this.tag)) {
        return
      }
      point.insertAdjacentHTML('beforeend', `<${this.tag} />`)
    }
    return
  }
  ...
  */
  ...
  disconnectedCallback() {
    ...
    Component[__run__](['unmounted', 'requestUpdate'], this)
  }
  ...
  requestUpdate() {
    ...
    return new Promise((resolve, reject) => {
      if (this.render && container) {
        resolve(renderer(this.render(), container, {
          eventContext: this
        }))
      } else {
        reject(new Error('Please provide renderer (lit-html by default)'))
      }
    })
  }
}
